...
# TaskStatus values inlined from TM backend.models.postgis.statuses (standalone: no backend pkg)
...
def _set_project_counters_from_task_states(project_id: int):
    ...
    tasks_mapped = sum([n for s, n in tasks_statuses if s == 2])
    tasks_validated = sum([n for s, n in tasks_statuses if s == 4])
    tasks_bad_imagery = sum([n for s, n in tasks_statuses if s == 6])
    query = (
        "UPDATE projects "
        + "SET tasks_mapped ="
        + str(tasks_mapped)
